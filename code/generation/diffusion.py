...
class DDPM:
    ...
    def p_sample(self, xt, c, t):
        ...
        if c is None:
            eps_theta = self.eps_model(xt, None, t)
        else:
    ...
    def cal_loss(self, x0, c=None):  # (B, L, 1)
    ...
    def sample(self, weight_path, n_samples, condition=None, output_dir="generation/ddpm"):
        """
        Generate samples, supports conditional and unconditional generation
        Args:
            weight_path: Model weight path
            n_samples: Number of samples to generate
            condition: Condition information, None for unconditional generation
        ...
        with torch.no_grad():
            ...
            for i in range(n_samples):
                ...
                for j in range(0, self.n_steps, 1):
                    t = torch.ones(1, dtype=torch.long, device=self.opt.device) * (self.n_steps - j - 1)
                    if condition is not None:
                        c = torch.from_numpy(condition).type(torch.float32).view(1, -1).to(self.opt.device)
                        x = self.p_sample(x, c, t)
                    else:
                        x = self.p_sample(x, None, t)
                
                # Post-process generated samples
                x_gen = x.squeeze().detach().cpu().numpy()
                x_gen = sig.medfilt(x_gen, 5)
                
                # Ensure values are positive
                x_gen = np.maximum(x_gen, 0)
                ...
                plt.plot(x_gen)
                plt.title(f'Generated Power Curve #{i}')
            ...
            print(f"All {n_samples} samples saved to {output_dir}")
    ...
    def train(self):
        ...
        for epoch in range(self.opt.n_epochs):
            ...
            for i, data in enumerate(self.data_loader):
                ...
                # Support unconditional training
                if "condition" in data and self.opt.cond_flag == "conditional":
                    c = data["condition"].to(self.opt.device)
                    loss = self.cal_loss(x0, c)
                else:
                    # Unconditional generation
                    loss = self.cal_loss(x0, None)
            ...
            save_dir = f"weights/{self.opt.model_name}/{self.opt.network}/{self.opt.level}/{self.opt.cond_flag}"
