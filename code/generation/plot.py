...
def load_real_scenario(source, day_number=100, seq_len=96):
    ...
    if source.endswith('.csv'):
        ...
        if start_idx >= len(df):
            print(f"Warning: The specified number of days is beyond the data range. Will use day 1 instead.")
            start_idx = 0
...
def find_closest_scenario(real_scenario_data, generated_scenarios_folder):
    ...
    Returns:
    numpy.ndarray: The closest generated scenario data, or None if not found.
    """
...
def plot_combined_closest_scenarios(real_scenario_data, models, day_number):
    ...
    # 3. Set ticks and borders (Thicken border lines)
    ax.tick_params(axis='both', which='major', labelsize=24, direction='in', width=2, length=8)
    for spine in ax.spines.values():
        spine.set_edgecolor('black')
        spine.set_linewidth(2)  # Thicken outer frame lines
...
def main():
    ...
    parser.add_argument('--day', type=int, default=30, help='The day number to select as the real scenario (starting from 1).')

    args = parser.parse_args()

    # Define the list of models to compare, mapping to display names used for finding styles
    models_to_analyze = {
        'gmm': 'GMM',
        'aae': 'VAE',
        'gan': 'GAN',
